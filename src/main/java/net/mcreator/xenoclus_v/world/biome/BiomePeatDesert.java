...
import net.mcreator.xenoclus_v.block.BlockUmbraPeat;
import net.mcreator.xenoclus_v.ElementsXenoclusOne;

import java.util.Random;

@ElementsXenoclusOne.ModElement.Tag
public class BiomePeatDesert extends ElementsXenoclusOne.ModElement {
	@GameRegistry.ObjectHolder("xenoclus_v:peatdesert")
	public static final BiomeGenCustom biome = null;
	public BiomePeatDesert(ElementsXenoclusOne instance) {
		super(instance, 454);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Dead Lands").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-10079488).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("peatdesert");
			topBlock = BlockUmbraPeat.block.getDefaultState();
			fillerBlock = BlockUmbraPeat.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockUmbraPeat.block.getDefaultState().getBlock() || ground == BlockUmbraPeat.block.getDefaultState().getBlock())
							&& (ground2 == BlockUmbraPeat.block.getDefaultState().getBlock()
									|| ground2 == BlockUmbraPeat.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockUmbraPeat.block.getDefaultState(), 2);
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == BlockUmbraPeat.block.getDefaultState().getBlock()
					|| blockType == BlockUmbraPeat.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockUmbraPeat.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockUmbraPeat.block.getDefaultState());
		}
