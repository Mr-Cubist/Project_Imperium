...
import net.mcreator.xenoclus_v.block.BlockAutumnLeaves;
import net.mcreator.xenoclus_v.ElementsXenoclusOne;

import java.util.Random;

@ElementsXenoclusOne.ModElement.Tag
public class BiomeAutumn extends ElementsXenoclusOne.ModElement {
	@GameRegistry.ObjectHolder("xenoclus_v:autumn")
	public static final BiomeGenCustom biome = null;
	public BiomeAutumn(ElementsXenoclusOne instance) {
		super(instance, 453);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Autumn").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-13369600).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("autumn");
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockUmbraGrass.block.getDefaultState().getBlock()
							|| ground == BlockUmbraSoil.block.getDefaultState().getBlock())
							&& (ground2 == BlockUmbraGrass.block.getDefaultState().getBlock()
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockAutumnLeaves.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockAutumnLeaves.block.getDefaultState());
										}
									}
								}
							}
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.LOG2.getStateFromMeta(1).getBlock()
					|| blockType == BlockAutumnLeaves.block.getDefaultState().getBlock()
					|| blockType == BlockUmbraGrass.block.getDefaultState().getBlock()
