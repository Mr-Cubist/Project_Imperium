...
public class StructureFrozenReefBig extends ElementsXenoclusOne.ModElement {
	public StructureFrozenReefBig(ElementsXenoclusOne instance) {
		super(instance, 631);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 7999) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = BlockBorealStone.block.getDefaultState();
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "frozenprismarineone"));
				if (template == null)
					return;
