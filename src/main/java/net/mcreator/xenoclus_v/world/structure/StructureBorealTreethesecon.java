...
public class StructureBorealTreethesecon extends ElementsXenoclusOne.ModElement {
	public StructureBorealTreethesecon(ElementsXenoclusOne instance) {
		super(instance, 795);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((random.nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = BlockBorealarkSnowblock.block.getDefaultState();
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "borealtree"));
				if (template == null)
					return;
