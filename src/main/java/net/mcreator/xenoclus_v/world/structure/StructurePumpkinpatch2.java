...
public class StructurePumpkinpatch2 extends ElementsXenoclusOne.ModElement {
	public StructurePumpkinpatch2(ElementsXenoclusOne instance) {
		super(instance, 474);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 7976) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = BlockUmbraGrass.block.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				require = BlockUmbraPeat.block.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "pumpkinpatch2"));
				if (template == null)
					return;
