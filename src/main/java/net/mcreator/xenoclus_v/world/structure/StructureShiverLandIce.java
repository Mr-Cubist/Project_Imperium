...
public class StructureShiverLandIce extends ElementsXenoclusOne.ModElement {
	public StructureShiverLandIce(ElementsXenoclusOne instance) {
		super(instance, 566);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldBorealark.DIMID) {
			...
			isNetherType = WorldBorealark.NETHER_TYPE;
		}
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 9899) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = BlockSnowyPermafrost.block.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				require = BlockJaggedIce.block.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				if (!blockCriteria)
					continue;
				...
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("xenoclus_v:shiverlands")))
					biomeCriteria = true;
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("xenoclus_v:insanemountains")))
					biomeCriteria = true;
				if (!biomeCriteria)
					continue;
				if (world.isRemote)
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "iceball"));
				if (template == null)
					return;
