...
import net.mcreator.xenoclus_v.block.BlockRotGranite;
import net.mcreator.xenoclus_v.ElementsXenoclusOne;

import java.util.Random;

@ElementsXenoclusOne.ModElement.Tag
public class StructureUndergroundUmbraHouse extends ElementsXenoclusOne.ModElement {
	public StructureUndergroundUmbraHouse(ElementsXenoclusOne instance) {
		super(instance, 478);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((random.nextInt(1000000) + 1) <= 6000) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = BlockRotGranite.block.getDefaultState();
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "shadedhouseu"));
				if (template == null)
					return;
