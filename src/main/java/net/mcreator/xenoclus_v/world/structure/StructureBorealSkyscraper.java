...
public class StructureBorealSkyscraper extends ElementsXenoclusOne.ModElement {
	public StructureBorealSkyscraper(ElementsXenoclusOne instance) {
		super(instance, 729);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 158009) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = BlockSnowySoftStone.block.getDefaultState();
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "building"));
				if (template == null)
					return;
