...
public class StructureCreepaliskDungeon extends ElementsXenoclusOne.ModElement {
	public StructureCreepaliskDungeon(ElementsXenoclusOne instance) {
		super(instance, 503);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 3994) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = Blocks.STONE.getDefaultState();
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "creepaliskdungeon"));
				if (template == null)
					return;
