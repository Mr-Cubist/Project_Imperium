...
public class StructureFrozenReefSmall extends ElementsXenoclusOne.ModElement {
	public StructureFrozenReefSmall(ElementsXenoclusOne instance) {
		super(instance, 630);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 8999) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "frozenreef"));
				if (template == null)
					return;
