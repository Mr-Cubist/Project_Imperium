...
public class StructurePaintedMahogany extends ElementsXenoclusOne.ModElement {
	public StructurePaintedMahogany(ElementsXenoclusOne instance) {
		super(instance, 544);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 120950) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "paintedmahogany"));
				if (template == null)
					return;
