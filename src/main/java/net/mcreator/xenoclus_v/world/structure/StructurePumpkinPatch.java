...
public class StructurePumpkinPatch extends ElementsXenoclusOne.ModElement {
	public StructurePumpkinPatch(ElementsXenoclusOne instance) {
		super(instance, 461);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((random.nextInt(1000000) + 1) <= 9000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				require = BlockAutumnDIrt.block.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "pumpkinpatch"));
				if (template == null)
					return;
