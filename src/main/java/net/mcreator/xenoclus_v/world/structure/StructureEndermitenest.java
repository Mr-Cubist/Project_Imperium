...
public class StructureEndermitenest extends ElementsXenoclusOne.ModElement {
	public StructureEndermitenest(ElementsXenoclusOne instance) {
		super(instance, 669);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 9830) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "endermitenest"));
				if (template == null)
					return;
