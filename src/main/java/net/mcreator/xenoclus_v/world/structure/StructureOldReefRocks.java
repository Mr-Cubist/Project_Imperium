...
public class StructureOldReefRocks extends ElementsXenoclusOne.ModElement {
	public StructureOldReefRocks(ElementsXenoclusOne instance) {
		super(instance, 652);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldAzathoth.DIMID) {
			...
			isNetherType = WorldAzathoth.NETHER_TYPE;
		}
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 10015) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = BlockWhiteSand.block.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				require = BlockGreenAlgeaGrass.block.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				if (!blockCriteria)
					continue;
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "oldreefrocks"));
				if (template == null)
					return;
