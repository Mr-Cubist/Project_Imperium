...
public class StructurePaintedTreeNormal extends ElementsXenoclusOne.ModElement {
	public StructurePaintedTreeNormal(ElementsXenoclusOne instance) {
		super(instance, 728);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 131105) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = BlockPaintedGrass.block.getDefaultState();
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "paintedtree"));
				if (template == null)
					return;
