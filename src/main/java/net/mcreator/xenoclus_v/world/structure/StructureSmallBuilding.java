...
public class StructureSmallBuilding extends ElementsXenoclusOne.ModElement {
	public StructureSmallBuilding(ElementsXenoclusOne instance) {
		super(instance, 576);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 159000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "smallbuilding"));
				if (template == null)
					return;
