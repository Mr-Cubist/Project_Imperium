...
public class StructurePrismarineHouse extends ElementsXenoclusOne.ModElement {
	public StructurePrismarineHouse(ElementsXenoclusOne instance) {
		super(instance, 673);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 9999) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = BlockPastelReefGrass.block.getDefaultState();
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("xenoclus_v", "prismarinehouse"));
				if (template == null)
					return;
