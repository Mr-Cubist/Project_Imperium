...
public class BlockSmallHIll extends ElementsXenoclusOne.ModElement {
	@GameRegistry.ObjectHolder("xenoclus_v:smallhill")
	public static final Block block = null;
	public BlockSmallHIll(ElementsXenoclusOne instance) {
		super(instance, 277);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0, new ModelResourceLocation("xenoclus_v:smallhill", "inventory"));
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("xenoclus_v:autumn")))
			biomeCriteria = true;
		if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("xenoclus_v:umbramountains")))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		for (int i = 0; i < 9; i++) {
	...
	public static class BlockCustomFlower extends BlockFlower {
		public BlockCustomFlower() {
			...
			setResistance(2.5F);
			setLightLevel(0F);
			setUnlocalizedName("smallhill");
			setRegistryName("smallhill");
		}
